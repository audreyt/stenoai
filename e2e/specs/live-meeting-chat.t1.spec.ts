...
/**
 * T1 — renderer-only, mock IPC. Pins the "Ask about the live transcript" surface
 * (feat/apple-transcription): AskBar must stay enabled WHILE a recording is
 * active (not inert like the old "Chat available after recording"), route
 * submit to the LIVE transcript stream (useStreamingQuery.startLiveStream →
 * ipc().query.askLiveStream) instead of the processed-note backend, render
 * streamed chunks, and offer a stop/cancel affordance.
 *
 * Live wiring is fully mocked: `askLiveStream` is a fire-and-forget send (no-op
 * under mock IPC), so the spec injects chunk delivery through the renderer's own
 * `subscribeQueryStream` capture — no backend, no model. `subscribeQueryStream`
 * handlers are renderer-internal, so chunk delivery runs inside page.evaluate.
 * The queue/status machine is the stateful mock in app/e2e-mock-ipc.js;
 * STENOAI_E2E_MOCK_PARAKEET_INSTALLED keeps the live-transcript panel available
 * so recording does not force it open (which would hide the Ask bar) — the dock
 * stays on the pill + Ask bar.
 */
...
async function startInBackground(page: Page, name = "Epsilon Planning") {
  await page.evaluate(() => window.stenoai.recording.start(name));
