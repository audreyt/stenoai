...
test("recording coexists: pill docks next to an enabled live Ask bar, expands, stops to processing", async ({
  ...
  await expect(
    pill.getByRole("button", { name: "Show transcript" }),
  ).toBeVisible();
  await expect(
    pill.getByRole("button", { name: "Stop recording" }),
  ).toBeVisible();
  await expect(
    pill.getByRole("button", { name: "Pause recording" }),
  ).toHaveCount(0);
  ...
  await expect(
    page.getByPlaceholder("Ask about the live transcript…"),
  ).toHaveCount(0);
  ...
  await expect(
    panel.getByRole("button", { name: "Stop recording" }),
  ).toBeVisible();
  await expect(
    panel.getByRole("button", { name: "Pause recording" }),
  ).toHaveCount(0);
...
test("auto-pause rescue: Resume appears only when the system paused the recording", async ({
  ...
  await expect(
    pill.getByRole("button", { name: "Resume recording" }),
  ).toHaveCount(0);
...
test("whisper variant: compact pill has no expand and no pause", async ({
  launchApp,
}) => {
  const { page } = await launchApp({
    mockIpc: true,
...
test("stale note (continued): floating CTA reads Generate notes", async ({
  launchApp,
}) => {
