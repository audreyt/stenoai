...
Steno records microphone and system audio, builds a local transcript, and keeps the meeting open as a working surface while you talk. On macOS 26 and later, Apple's system-managed SpeechTranscriber handles live and batch transcription without downloading ASR weights. The Ask bar works during the meeting and queries finalized speech through the configured local (Apple Intelligence, Ollama, or a private server), cloud, or organisation AI provider.

Audio never leaves the Mac for transcription. Summary and chat prompts use Apple Intelligence on-device when available; otherwise they follow the configured Ollama, private-server, cloud, or organisation route.
...
- **On-device Apple Intelligence** for summary and chat when available, with Ollama / private-server / cloud fallbacks.
...
- **2026-08-24: Apple Intelligence summaries.** On-device SystemLanguageModel is the default for summaries and live chat when the Mac has it; Ollama and private-server profiles remain available.
...
- **Apple-native live transcription** — SpeechTranscriber provides volatile and final text, timestamps, confidence, and system-managed locale assets on macOS 26+. Parakeet TDT v3 (MLX) and Whisper serve as fallbacks on Apple Silicon, older macOS versions, and Windows. Granola-style chat-bubble view with `[You]` vs `[Others]` attribution live during recording and on the final transcript.
...
- **Ask your meetings (live chat)** — Ask the in-progress live transcript while recording, query a single finished note, or search your library via the Chat tab. Live questions snapshot finalized segments and stream answers using the configured local (Apple Intelligence, Ollama, Ornith), cloud, or organisation AI provider.
- **Multi-language (45 native, 25 live Parakeet, 99 Whisper)** — Apple SpeechTranscriber covers 45 locales; Parakeet covers 25 European languages with live transcription; Whisper handles 99 languages post-stop.
- **Markdown ownership** — Summaries and transcripts save as clean Markdown you can edit, search, or sync to whatever knowledge base you live in (including Obsidian sync).
...
**Summarization & Live-Meeting Chat Models:**
...
- `ornith-1.5:9b`: Self-hosted Ollama-compatible model for summary and chat via loopback
## Future Roadmap
