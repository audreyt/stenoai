...
export function useStreamingQuery() {
   ...
   *  Routes to ipc().query.askLiveStream which the main process handles via
   *  query-live-transcript-stream → the Ollama-compatible backend. Chunks
   *  land on the same query-chunk/query-done/query-error channels so the
   *  same subscribeQueryStream subscription drives the UI. */
